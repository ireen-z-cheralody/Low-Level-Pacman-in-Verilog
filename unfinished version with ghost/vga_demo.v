...
//change: resolution was set to 160x120
	...
	//change: added new wires that would draw the three coins, the ghost, the game won and game over screen
	//change: changed O1 wires to be pacman
	wire [nX-1:0] pacman_x, coin1_x, coin2_x, coin3_x, gameWon_x, ghost_x, gameOver_x, MUX_x;    // x coordinate multiplexer
	wire [nY-1:0] pacman_y, coin1_y, coin2_y, coin3_y, gameWon_y, ghost_y, gameOver_y, MUX_y;    // y coordinate multiplexer
	wire [COLOR_DEPTH-1:0] pacman_color, coin1_color, coin2_color, coin3_color, gameWon_color, gameOver_color, ghost_color, MUX_color; // color multiplexer
	wire pacman_write, coin1_write, coin2_write, coin3_write, gameWon_write, ghost_write, gameOver_write, MUX_write; // write control multiplexer
	...
	//change: add more done wires for the new objects (coins and game won screen)
   	wire pacman_done, coin1_done, coin2_done, coin3_done, gameWon_done, gameOver_done, ghost_done, done;             // object move completed
	
	//change: object selector is now a 3 bit wire called whichObject
   	reg [2:0] whichObject;          
   	wire [2:0] pacman_dir, ghost_dir;              	//change: used to set direction for moving pacman & the ghost
   	reg [1:0] y_Q, Y_D;             		// FSM, used to control drawing of objects

   	wire Resetn, KEY0, KEY3;        		// Reset, and synchronized versions of KEY3
   	wire PS2_CLK_S, PS2_DAT_S;      		// synchronized versions of PS2 signals

	//change: synchronize KEY3
	sync S1 (~KEY[3], Resetn, CLOCK_50, KEY3);
	
	//change: assigned reset to KEY1
	assign Resetn = KEY[1];
	...
	//change: wire to indicate that game is over
	wire gameOver;
	
	//change: checks if pacman is colliding with the ghost at any point
	ghost_pacman_collision_checker GPCC(pacman_x, pacman_y, ghost_x, ghost_y, gameOver);
	...
	//change: send KEY0 to pulse immediately on reset so that pacman is drawn 
	pulse P4 (CLOCK_50, Resetn, 1, KEY0);	
	...
	//change: wires to help assign ghost's direction of movement
	wire ghostUp, ghostDown, ghostLeft, ghostRight;
	
	//change: ghost movement module that sets direction based on current ghost coordinates
	ghost_mover GM(CLOCK_50, Resetn, ghost_x, ghost_y, ghostUp, ghostDown, ghostLeft, ghostRight, KEY3);
	
	parameter LEFT = 3'b000 /*'d'*/, RIGHT = 3'b011/*'a'*/, UP = 3'b001/*'w'*/, DOWN = 3'b010/*'s'*/;
	reg temp_ghost_dir;
	
	//change: set ghost direction based on the output of the ghost movement module
	always@(posedge CLOCK_50)
		if (ghostUp)
			...
			temp_ghost_dir <= RIGHT;
		else 
			temp_ghost_dir <= 3'b100;
			
	assign ghost_dir = temp_ghost_dir;
		

	//change: use a flipper to switch between selecting ghost and pacman to be drawn
	wire ghostOrPacman;
	flipper_7cycle GOP (CLOCK_50, Resetn, ghostOrPacman);
	...
	always @(posedge CLOCK_50)
		if (!Resetn | KEY0)
			...
			whichObject <= 3'b101;  //change: select ghost to be drawn when KEY3 is pressed
		...
		else if (step & (gameWon == 0 & gameOver == 0)) //change: if the game is not over yet then switch between selecting ghost or pacman
		begin
			if (ghostOrPacman)
				whichObject <= 3'b000;
			else
			...
			whichObject <= 3'b110; //change: pick game over screen if game is over
	...
	object O1 (Resetn, CLOCK_50, KEY0, object_sel & step & ghostOrPacman, pacman_dir, pacman_x, pacman_y, 
		...
		//Ireen: a copy of the above object with changes to draw the coin (still being debugged)
	...
	object O5 (Resetn, CLOCK_50, gameWon & (coin1_done | coin2_done | coin3_done), 1'b0, 3'b100, gameWon_x, gameWon_y, 
		...
		defparam O5.INIT_FILE = "./MIF/gamewin.mif";
	
	//change: ghost (different object module)
	object_ghost O6 (Resetn, CLOCK_50, KEY3, !ghostOrPacman, ghost_dir, ghost_x, ghost_y, 
               ghost_color, ghost_write, ghost_done, ghostStableX, ghostStableY);
		defparam O6.XOFFSET = 154;
		defparam O6.YOFFSET = 72;
      		defparam O6.nX = nX;
		defparam O6.nY = nY;
		defparam O6.COLOR_DEPTH = COLOR_DEPTH;
		defparam O6.INIT_FILE = "./MIF/ghost.mif";

	//change: game over screen
	object O7 (Resetn, CLOCK_50, gameOver, 1'b0, 3'b100, gameOver_x, gameOver_y, 
               gameOver_color, gameOver_write, gameOver_done, dummyStableX, dummyStableY);
		defparam O7.RESOLUTION = RESOLUTION;
		defparam O7.xOBJ = 7;
		defparam O7.yOBJ = 6;
      		defparam O7.nX = nX;
		defparam O7.nY = nY;
		defparam O7.COLOR_DEPTH = COLOR_DEPTH;
		defparam O7.INIT_FILE = "./MIF/gamelose.mif";

		//change: add more done signals for each object
		assign done = pacman_done | coin1_done | coin2_done | coin3_done | gameWon_done | gameOver_done | ghost_done;
	 ...
	 //change: these assign statements decide which object (1, 2, 3, 4 or 5) get drawn based on the value of whichObject
		assign MUX_x =
		 ...
		 (whichObject == 3'b100) ? gameWon_x:
		 (whichObject == 3'b101) ? ghost_x : 
		 (whichObject == 3'b110) ? gameOver_x: 1'b0;
		...
		assign MUX_y =
		 ...
		 (whichObject == 3'b100) ? gameWon_y : 
		 (whichObject == 3'b101) ? ghost_y : 
		 (whichObject == 3'b110) ? gameOver_y: 1'b0;
		...
		assign MUX_color =
		 ...
		 (whichObject == 3'b100) ? gameWon_color : 
		 (whichObject == 3'b101) ? ghost_color: 
		 (whichObject == 3'b110) ? gameOver_color: 1'b0;
		...
		assign MUX_write =
		 ...
		 (whichObject == 3'b100) ? gameWon_write : 
		 (whichObject == 3'b101) ? ghost_write : 
		 (whichObject == 3'b110) ? gameOver_write: 1'b0;
